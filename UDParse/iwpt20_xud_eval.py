...
#
# Original authors: Milan Straka, Martin Popel <surname@ufal.mff.cuni.cz>
#
# Changelog:
...
# - [1 Sep 2021]  Version 1.3: output detailed results on every UPOS, XPOS and deprel (Johannes Heinecke)
...
def process_enhanced_deps(deps):
    edeps = []
    if deps != "_":  # JHE accept conllu without deps
...
def load_conllu(file_or_fnlist, treebank_type={}):
    ...
    class UDWord:
        ...
        def __repr__(self):
            #return "%d--%d" % (self.span.start,self.span.end)
            return "%s" % self.columns

    ud = UDRepresentation()

    ifplist = []
    if isinstance(file_or_fnlist, list):
        for fn in file_or_fnlist:
            ifplist.append(open(fn))
    else:
        ifplist.append(file_or_fnlist)

    # Load the CoNLL-U file(s)
    for ifp in ifplist:
        ...
        while True:
            ...
            if not line:
                ...
                for word in ud.words[sentence_start:]:
                    ...
                    for (head, steps) in word.columns[DEPS]:  # (3,['conj:en','obj:voor'])
                        if "." in head:
                            # JHE: ignore empty nodes since we did not collapse empty nodes beforehand
                            continue
                        hd = int(head)
                    ...
                    if treebank_type.get("no_gapping", 0):  # enhancement 1
                        processed_deps = []
                    ...
                    if treebank_type.get("no_shared_parents_in_coordination", 0):  # enhancement  2
                    ...
                    if treebank_type.get("no_shared_dependents_in_coordination", 0):  # enhancement  3
                        processed_deps = []
                        for (hd, steps) in enhanced_deps:
                            duplicate = 0
                    ...
                    if treebank_type.get("no_control", 0):  # enhancement 4
                        processed_deps = []
                        for (parent, steps) in enhanced_deps:
                            include = 1
                    ...
                    if treebank_type.get("no_external_arguments_of_relative_clauses", 0):  # enhancement 5
                        processed_deps = []
                        for (parent, steps) in enhanced_deps:
                            if steps[0] == "ref":
                    ...
                    if treebank_type.get("no_case_info", 0):  # enhancement number 6
                        processed_deps = []
                        for (hd, steps) in enhanced_deps:
                            processed_steps = []
                            for dep in steps:
            ...
            if "." in columns[ID]:
                # JHE ignore (we did not collapse empty nodes before hand)
                continue
            ...
            if "-" in columns[ID]:
                ...
                for _ in range(start, end + 1):
                    word_line = _decode(ifp.readline().rstrip("\r\n"))
...
def evaluate(gold_ud, system_ud, ofp=None):
    class Score:
        ...
        def __str__(self):
            return "%9.2f %s %9.2f %s %9.2f %s %7d %s %7d %s %7d %s" % \
                   (100*self.precision,  colsep,
                    100*self.recall,  colsep,
                    100*self.f1,  colsep,
                    self.gold_total,  colsep,
                    self.system_total,  colsep,
                    self.correct,  colend)
    ...
    def alignment_score(alignment, key_fn=None, filter_fn=None, metric=None):
        # print("==============================", metric)
        ...
        if col != None:
            if col == FEATS:
                for w in alignment.gold_words:
                    #print("zzzg", w.columns[col])
                    for g in w.columns[col].split("|"):
                        if not g in categorieGold:
                            categorieGold[g] = 0
                        categorieGold[g] += 1
                for w in alignment.system_words:
                    #print("zzzs", w.columns[col])
                    for s in w.columns[col].split("|"):
                        if not s in categorieSystem:
                            categorieSystem[s] = 0
                        categorieSystem[s] += 1
                ...
                for w in alignment.gold_words:
                    #print("zzzg", w.columns[col])
                    if not w.columns[col] in categorieGold:
                        categorieGold[w.columns[col]] = 0
                    categorieGold[w.columns[col]] += 1
            for w in alignment.system_words:
                # print("zzzs", w.columns[col])
                if not w.columns[col] in categorieSystem:
                    categorieSystem[w.columns[col]] = 0
                categorieSystem[w.columns[col]] += 1

        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                # print(metric, "aaa", key_fn(words.gold_word, gold_aligned_gold))
                if metric in ["LAS", "CLAS", "MLAS", "BLEX"]:
                    goldcat = key_fn(words.gold_word, gold_aligned_gold)[1]
                    syscat = key_fn(words.system_word, gold_aligned_system)[1]
                else:  # elif metric and "POS" in metric:
                    goldcat = key_fn(words.gold_word, gold_aligned_gold)
                    syscat = key_fn(words.system_word, gold_aligned_system)

                if metric == "FEATS":
                    for c in [goldcat, syscat]:
                        #print("cat",c)
                        for f in c.split("|"):
                            if not f in categorieCorrect:
                                categorieCorrect[f] = 0

                else:
                    for c in [goldcat, syscat]:
                        #print("cat",c)
                        if not c in categorieCorrect:
                            categorieCorrect[c] = 0

                #print("bbbb",
                #      #words.gold_word.columns,
                #      key_fn(words.gold_word, gold_aligned_gold),
                #      " =?=  ",
                #      #words.system_word.columns,
                #      key_fn(words.system_word, gold_aligned_system), sep= "\n")
                # key_fn() returns the relevant columns from a conllu line
                if metric == "FEATS":
                    goldfeats = key_fn(words.gold_word, gold_aligned_gold)
                    sysfeats = key_fn(words.system_word, gold_aligned_system)
                    if goldfeats == sysfeats:
                        correct += 1
                        for f in goldfeats.split("|"):
                            categorieCorrect[f] += 1
                    else:
                        goldfeats = set(goldfeats.split("|"))
                        for f in sysfeats.split("|"):
                            if f in goldfeats:
                                categorieCorrect[f] += 1
                    ...
                    if key_fn(words.gold_word, gold_aligned_gold) == key_fn(words.system_word, gold_aligned_system):
                        correct += 1
                        categorieCorrect[c] += 1

        s = Score(gold, system, correct, aligned)
        if ofp:
            if metric in ["FEATS", "UPOS", "XPOS", "LAS"]:
                if uselatex:
                    print("\\begin{tabular}{l r r r   r r r r}", file=ofp)
                else:
                    print("%s =================" % metric, file=ofp)
                print("           \t%s     P     %s     R     %s     F1    %s nb gold %s nb system %s nb correct %s" % (colsep,colsep,colsep,colsep,colsep,colsep,colend), file=ofp)
                #print("::%-8s\t|" % metric, s)

                #print("cg", categorieGold)
                #print("cs", categorieSystem)
                #print("cc", categorieCorrect)
                sumG = sumS = sumC = 0
                for c in sorted(categorieGold):
                    if c == "": continue
                    #print(c,categorieGold[c], categorieSystem.get(c, 0), categorieCorrect.get(c, 0))
                    sc = Score(categorieGold[c], categorieSystem.get(c, 0), categorieCorrect.get(c, 0), None)
                    sumG += categorieGold.get(c, 0)
                    sumS += categorieSystem.get(c, 0)
                    sumC += categorieCorrect.get(c, 0)
                    print("%-10s\t%s" % (c,colsep), sc, file=ofp)
                total = Score(sumG, sumS, sumC, None)
                print(">> total   \t%s" % colsep, total, file=ofp)
                if uselatex:
                    print("\\end{tabular}", file=ofp)
    ...
    def enhanced_alignment_score(alignment, EULAS):
        ...
        for gold_word in alignment.gold_words :
            ...
            #print("eee", gold_word.columns[DEPS])
            gold += len(gold_word.columns[DEPS])
            for eud in gold_word.columns[DEPS]:
                if eud[1][0] not in categorieGold:
                    categorieGold[eud[1][0]] = 0
                categorieGold[eud[1][0]] += 1
        system = 0
        for system_word in alignment.system_words :
            #print("eee", gold_word.columns[DEPS])
            system += len(system_word.columns[DEPS])
            for eud in system_word.columns[DEPS]:
                if eud[1][0] not in categorieSystem:
                    categorieSystem[eud[1][0]] = 0
                categorieSystem[eud[1][0]] += 1
...
#            else:
#                print("%s =================" % "ELAS")
#            print("           \t%s     P     %s     R     %s     F1    %s nb gold %s nb system %s nb correct %s" % (colsep,colsep,colsep,colsep,colsep,colsep,colend) )
#
#
#            for c in sorted(categorieGold):
#                #print(c,categorieGold[c], categorieSystem.get(c, 0), categorieCorrect.get(c, 0))
#                sc = Score(categorieGold[c], categorieSystem.get(c, 0), categorieCorrect.get(c, 0), None)
#                print("%-10s\t%s" % (c, colsep), sc)
#            print(">> total   \t%s" % colsep, s)
#            print()
#            print()
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        raise UDError(
            ...
            + "First 20 differing characters in gold file: '{}' and system file: '{}'".format(
                "".join(map(_encode, gold_ud.characters[index : index + 20])), "".join(map(_encode, system_ud.characters[index : index + 20]))
            )
        )
    ...
    #    # does not work when tokenisation differs
    #    #print("eee1", len(alignment.gold_words))
    #    #print("eee2", len(alignment.system_words))
    #    deprelsOK = {} # deprel and head OK
    #    deprelsKO = {} # deprel and/or head KO
    #    for i,mw in enumerate(alignment.matched_words):
    #        gw = mw.gold_word
    #        sw = mw.system_word
    #        print(i,gw.columns)
    #        print(i,sw.columns)
    #
    #        if gw.columns[DEPREL] == "qqqpunct":
    #            continue
    #        if not gw.columns[DEPREL] in deprelsOK:
    #            deprelsOK[gw.columns[DEPREL]] = 0
    #            deprelsKO[gw.columns[DEPREL]] = 0
    #        if gw.columns[DEPREL] == sw.columns[DEPREL] and gw.columns[HEAD] == sw.columns[HEAD]:
    #            deprelsOK[gw.columns[DEPREL]] += 1
    #        else:
    #            deprelsKO[gw.columns[DEPREL]] += 1
    #    ok = 0
    #    ko = 0
    #    for d in deprelsKO:
    #        print("%s\t%d\t%d" % (d, deprelsOK[d], deprelsKO[d]))
    #        ok += deprelsOK[d]
    #        ko += deprelsKO[d]
    #    #print("\t%d\t%d\t%.2f" % (ok, ko, 100*ok/(ko+ok)))
    #    print("\t%d\t%d\tp:%.2f\tr:%.2f" % (ok, ko, 100*ok/len(alignment.system_words), 100*ok/len(alignment.gold_words)))
    #    print("eee3", len(alignment.matched_words_map))
    #    print("eee4", len(alignment.matched_words))
    #    #print("eee4", alignment.matched_words)
    #    #for i,mw in enumerate(alignment.matched_words):
    #    #    print(i, mw.gold_word.columns)
    #    #    print(i, mw.system_word.columns)
    ...
    return {
        ...
        "UPOS": alignment_score(alignment, lambda w, _: w.columns[UPOS], metric="UPOS"),
        "XPOS": alignment_score(alignment, lambda w, _: w.columns[XPOS], metric="XPOS"),
        "UFeats": alignment_score(alignment, lambda w, _: w.columns[FEATS], metric="FEATS"),
        ...
        "LAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]), metric="LAS"),
        # include enhanced DEPS score -- GB
        "ELAS": enhanced_alignment_score(alignment, 0),
        "EULAS": enhanced_alignment_score(alignment, 1),
        "CLAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]), filter_fn=lambda w: w.is_content_deprel, metric="CLAS"),
...
def evaluate_wrapper(args, ofp=None):
    treebank_type = {}
    ...
    return evaluate(gold_ud, system_ud, ofp)
...
def main():
    ...
    parser.add_argument("--latex", "-L", default=False, action="store_true",
                        help="format tables for LaTeX.")
    ...
    # Evaluate
    evaluation = evaluate_wrapper(args, ofp=sys.stdout)
    ...
    if not args.verbose and not args.counts:
        ...
        if args.latex:
            print("\\begin{tabular}{l r r r   r r r r}")
        if args.counts:
            print("Metric     %s Correct   %s      Gold %s Predicted %s Aligned%s" % (colsep,colsep,colsep,colsep,colend))
        else:
            print("Metric     %s Precision %s    Recall %s  F1 Score %s AligndAcc%s" % (colsep,colsep,colsep,colsep,colend))
        ...
        for metric in["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "UAS", "LAS", "ELAS", "EULAS", "CLAS", "MLAS", "BLEX"]:
            if args.counts:
                print("{:11}{}{:10} {}{:10} {}{:10} {}{:10}{}".format(
                    metric,  colsep,
                    evaluation[metric].correct,  colsep,
                    evaluation[metric].gold_total,  colsep,
                    evaluation[metric].system_total,  colsep,
                ...
                print("{:11}{}{:10.2f} {}{:10.2f} {}{:10.2f} {}{}{}".format(
                    metric,  colsep,
                    100 * evaluation[metric].precision,  colsep,
                    100 * evaluation[metric].recall,  colsep,
                    100 * evaluation[metric].f1,  colsep,
